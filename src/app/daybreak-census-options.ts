...
function StringUnion<UnionType extends string>(
  ...values: UnionType[]
): {
...
const StringFilterMatch = StringUnion(
  'equals',
  'startsWith',
  'contains',
  'notEqual'
...
const NumberFilterMatch = StringUnion(
  'equals',
  'lessThan',
  'lessThanOrEqual',
  'greaterThan',
...
function checkFilter(
  ...
  switch (typeof filter.value) {
    ...
    default:
      throw new Error(
        'Only numbers and strings can be used as filter queries.'
      );
  }
  return filter;
}
...
type QueryOptions = Exclude<
  keyof CensusUrlOptions,
  'serviceId' | 'format' | 'verb' | 'namespace' | 'collection' | 'identifier'
>;
type Mappings = {
  [K in QueryOptions]-?:
    | string
    | ((url: URL, value: CensusUrlOptions[K]) => void);
};
const mappings: Mappings = {
  filter: (url, value) =>
    value.forEach((filter) => {
      ...
      url.searchParams.append(
        filter.field,
        filterModifiers[filter.match] + filter.value
      );
    }),
  show: 'c:show',
  hide: 'c:hide',
  ...
  has: 'c:has',
  resolve: (url, value) => {
    const resolves = value.map((resolve) =>
      resolve.show
        ? `${resolve.field}(${resolve.show.join(',')})`
        : resolve.field
    );
  ...
  tree: (url, trees) => {
    const treeParams = trees.map((tree) =>
      Object.entries(tree)
        // Keep those items that have a value (including false).
        .filter(([, value]) => value !== undefined)
        .map(
